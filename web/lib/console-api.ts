...
function translateText(text: string, locale: Locale = "zh-CN") {
  ...
  const replacements: Array<[RegExp, string]> = [
    ...
    [/Recent ingress decisions persisted in the configured datastore/gi, "最近持久化到当前配置数据存储的网关判定事件"],
    ...
    [/The approval lane now persists decisions in the configured datastore instead of remaining display-only\./gi, "审批通道现在会把决策持久化到当前配置的数据存储，而不是只做展示。"],
